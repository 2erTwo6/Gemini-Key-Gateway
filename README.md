...
- **安全拦截自动重试（防误报，默认关闭）**：检测到 `generateContent`/`streamGenerateContent` 响应被安全机制拦截（`promptFeedback.blockReason` 非空或候选 `finishReason` 为 SAFETY 等）时，自动在请求体 `contents` 末尾追加一条 `user: EOF` 消息后复用同一 Key 重试，利用上游「只检查最后一条 user 消息」的特性减少正常对话被误报拦截（需显式开启 `block_retry: true` 并设 `max_block_retries ≥ 1`）
...
网关在检测到拦截后，会在请求体 `contents` 末尾自动追加上述一条 user 消息并复用**同一 Key** 重试，无需客户端改动。检测兼容非流式 JSON、流式 SSE、JSON 数组与 gzip 压缩响应。
